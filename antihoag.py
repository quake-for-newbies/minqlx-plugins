...
class antihoag(minqlx.Plugin):
  ...
  def handle_player_spawn(self, player):
      players_waiting = [p for p in self.players() if p.team in "free"]
      if len(players_waiting) > 3:
          limit = int(self.get_cvar("qlx_maxWinStreak"))
          current_streak = int(self.winner_streak)
          if limit > 0 and current_streak >= limit and player.steam_id == self.winner_id:
            player.put('spectator')
            self.winner_streak = 0
            self.msg("{} has reached the consecutive win limit of {} and has been moved to spectators."
                     .format(player.name, limit))
          print(f"here we're comparing values : limit is {limit}, current streak is {current_streak}, and spawning player is {player.name}")
      else:
